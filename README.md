...
| `koware provider autoconfig [name]` | Pull remote config (no-arg: one primary profile per provider family) |
| `koware provider autoconfig --all` | Pull every remote profile/variant |
| `koware provider autoconfig --defaults` | Pull only profiles marked `default=true` (legacy mode) |
